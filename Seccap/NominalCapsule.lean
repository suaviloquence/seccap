...
inductive Multistep [DecidableEq 𝔸] : Capsule 𝔸 C → Capsule 𝔸 C → Prop where
| refl c : Multistep c c
| trans c₁ c₂ c₃ : Step c₁ c₂ → Multistep c₂ c₃ → Multistep c₁ c₃
...
open Capsule in
lemma preservation {c c': Capsule 𝔸 C} {Γ τ}
...
open Capsule in
lemma safety' [Infinite 𝔸] {c₁ c₂: Capsule 𝔸 C} {Γ τ} (hs: Step.Multistep c₁ c₂) (ht: HasType Γ c₁.t τ)
  (hσ : Γ.dom = c₁.σ.dom) (hΓ: ∀ {a} (ha: a ∈ Γ.dom), HasType Γ (c₁.σ.getVal a (hσ ▸ ha)) (Γ.getVal a ha)):
  c₂.t.1.value ∨ ∃ c₂', Step c₂ c₂' := by
  induction hs generalizing Γ with
  | refl c =>
    apply progress (σ := c.σ) at ht
    · rcases ht with hv|⟨t', ⟨σ', hs⟩⟩
      · left; exact hv
      · right; use ⟨t', σ'⟩
    · simp [← hσ]
  | trans c₁ c₂ c₃ hc hs ih =>
    apply preservation (hτ := ht) at hc
    obtain ⟨Γ', ⟨ht', ⟨hle, ⟨hσ', hΓ'⟩⟩⟩⟩ := hc hσ hΓ
    exact ih ht' hσ' hΓ'

open Capsule in
theorem safety [Infinite 𝔸] {t: Term.Ground 𝔸 C} {c τ} (hs: Step.Multistep ⟨t, []⟩ c) (ht: HasType [] t τ) :
  c.t.1.value ∨ ∃ c', Step c c' := by
  apply safety' hs ht <;> simp [Store.dom]
end Types
